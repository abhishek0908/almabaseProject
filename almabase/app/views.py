...
from .forms import PersonRegistration,PersonLogin,UserProfileForm,InterviewExperienceForm,JobFeedForm,Contactform
    ...
    if request.user.is_authenticated:
        if request.method=='POST':
            ...
            if fm.is_valid():
                experience = fm.save(commit=False)  # Create the object but don't save it yet
                experience.user = request.user  # Associate the logged-in user with the experience
                 # Save the object with the associated user
            ...
            try:
                ...
                messages.error(request, 'An error occurred While Posting the interview. Please try again later.')
...
def feedback(request):
    if request.method=='POST':
        ...
        if fm.is_valid():
            ...
            query  = fm.cleaned_data['suggestion']
            recipient_list= ['abhishekudiya09@gmail.com',]
            try:
    ...
    return  render(request,'app/feedback.html',{'forms':fm})